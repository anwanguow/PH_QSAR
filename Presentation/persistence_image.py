...
file = 'mols/aspirin.xyz'
D, elements = dist_mat(file)
myspecs = {"maxBD": r_max, "minBD": -0.1}
rips = Rips(maxdim=2)
...
Totalmatrix = [h0matrix, h1matrix, h2matrix]
pim = PI(pixels=[pixel_m, pixel_n], spread=sigma, specs=myspecs, kernel_type="gaussian", verbose=False)
imgs = pim.transform(Totalmatrix)

fig, ax = plt.subplots()
pim.show(imgs, ax=ax)
plt.savefig("PI_r_" + str(r_max) + "_white_background_inverted.png", dpi=300)
plt.show()