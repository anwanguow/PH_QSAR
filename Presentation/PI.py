...
class PI(TransformerMixin):
    ...
    def show(self, imgs, ax=None):
        ...
        for i, img in enumerate(imgs):
            # Normalize each image independently to its own [0, 1] range
            img_normalized = (img - np.min(img)) / (np.max(img) - np.min(img))
            # Invert the colors: light becomes dark and dark becomes light
            img_inverted = 1 - img_normalized
            # Overlay each diagram with some transparency (alpha)
